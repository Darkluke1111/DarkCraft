...
public class AdvRecipe implements ConfigurationSerializable {
     ...
     * @param shape         The shape of the crafting grid represented as a string of nine characters.
     * @param ingredientMap A Map that defines which character from the shape goes with which ingredient item.
     * @return The Advanced recipe.
     */
    public static AdvRecipe createRecipe(ItemStack result, String[] shape, Map<Character, MaterialData> ingredientMap) {
        AdvRecipe advRecipe = new AdvRecipe(result, shape, ingredientMap);
        return advRecipe;
    }
     ...
     * @return The AdvRecipe wrapper class for the recipe or null if the recipe wasn't an advanced one.
     */
    ...
    /**
     * Adds a CraftingEvent to the recipe. The order in which the events are added decides in which order they are
     * checked while the crafting process.
     *
    ...
    /**
     * Adds a List of CraftingEvents to the recipe. The order of the List will determine in which order the behaviors will be checked in the crafting process.
     *
    ...
    /**
     * Registers the Recipe. Without registering an Advanced Recipe it will not be craftable and also won't be shown in
     * the recipe selection views.
     */
    ...
    public AdvRecipe(Map<String, Object> map) {
        ...
        for (Map.Entry<String, Object> ent : ((Map<String, Object>) map.get("ingredients")).entrySet()) {
            String[] splitData = ((String) ent.getValue()).split(":");
            MaterialData matdata = new MaterialData(Material.getMaterial(splitData[0]), Byte.parseByte(splitData[1]));
        ...
        if(map.get("behaviors") != null) {
            for (Map.Entry<String, Object> beh : ((Map<String, Object>) map.get("behaviors")).entrySet()) {
