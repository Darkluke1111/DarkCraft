...
public class Selection {
    ...
    /**
     * Generates a Structure Object with the given name from the current selection, which can then be used as for crafting structures.
     */
    public Structure convertToStructure(String name) {
        return Structure.createStructureFromWorld(getMinimalLocation(), name, getXSpan(), getYSpan(), getZSpan());
    }

    //region Getter/Setter

    /**
     * Returns the Location of the selections corner with the lowest x,y and z value.
     * @return
     */
    public Location getMinimalLocation() {
        if(loc1 == null || loc2 == null) return null;
        return new Location(loc1.getWorld(), Math.min(loc1.getBlockX(), loc2.getBlockX()), Math.min(loc1.getBlockY(), loc2.getBlockY()), Math.min(loc1.getBlockZ(), loc2.getBlockZ()));
    }

    /**
     * Returns a 3-dimensional array with a spacial representation of the MaterialData in the Selection.
     */
        ...
        for(int y = 0 ; y < getYSpan() ; y++) {
            for(int x = 0 ; x < getYSpan() ; x++) {
                for(int z = 0 ; z < getYSpan() ; z++) {
                    ...
                    data[x][y][z] = new MaterialData(iterator.getBlock().getType(), iterator.getBlock().getData());
                }
            }
        }
        return data;
