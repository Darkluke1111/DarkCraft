...
public class MenuViewFactory {
  ...
  /**
   * Creates a view which lets a player select between a set of recipes. If he
   * clicks on a recipe, there will be generated a view for the selected recipe
   * and will be shown to the player.
   */
  public static MenuView createRecipeSelectionView(Set<AdvRecipe> recipes) {
    ...
    for (AdvRecipe recipe : recipes) {
      ...
      MenuItem menuItem = MenuItem
          .createItem(result.getData(), result.getAmount())
          .withDescription(description)
          .withLeftClickAction(
              (InventoryClickEvent e, MenuItem i) ->
                MenuViewFactory.createRecipeView(recipe)
                    .setParentView(recipeSelectionView)
                    .setLinkToParent(
                        new MaterialData(Material.REDSTONE_BLOCK), "Back to Recipe Selection")
