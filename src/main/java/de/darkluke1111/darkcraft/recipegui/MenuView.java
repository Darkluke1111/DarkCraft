...
public class MenuView {
  ...
  public MenuItem getItemAt(int x, int y) {
    if (y * columns + x >= items.length) {
      throw new IndexOutOfBoundsException(
          "Tried accessing Position (" + x + "|" + y + ") but Inventory is only 9x" + rows);
    }
  ...
  public MenuItem getItemAtIndex(int index) {
    if (index >= items.length) {
      throw new IndexOutOfBoundsException(
          "Tried accessing Index " + index + " but Inventory is only " + items.length + " big.");
    }
    return items[index];
  }

  /**
   * Checks whether there is a MenuItem at the given coordinates
  ...
  /**
   * Checks whether there is a MenuItem at the given index.
   * Index is counted like slotnumbers in inventories.
   */
  ...
  /**
   * Sets a MenuItem to the given coordinates in the MenuView
   * (x from left to right, y from top to bottom).
   */
  ...
  /**
   * Sets a MenuItem to the given index in the MenuView.
   * Index is counted like slotnumbers in inventories.
   */
  ...
  /**
   * Sets a parent view for the current view which enables setting
   * "back-buttons" with the "setLinkToParent" method.
   */
  ...
  /**
   * Inserts a "back-button" into the view witch closes the current
   * view on left-click and shows the parent view to the same player.
   * The Button is added to the bottom-right corner of the current
   * view, if this slot is empty, otherwise the button won't be added
   * and a warning will be printed instead into console.
   */
  public MenuView setLinkToParent(MaterialData icon, String linkName) {
    if (itemAtSlot(columns - 1, rows - 1)) {
      DarkCraft.instance.getLogger().warning(
          "Wasn't able to establish link icon because slot was already full!");
      return this;
    }
    if (parentView == null) {
    ...
    MenuItem.createItem(icon).withName(linkName).withLeftClickAction((e, i) ->
      parentView.show(e.getViewers().get(0))
  ...
  public void handleInteraction(InventoryInteractEvent ievent) {
    ...
    if (!(ievent instanceof InventoryClickEvent)) {
      return;
    }
